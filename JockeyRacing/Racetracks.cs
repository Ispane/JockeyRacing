...
{
    public partial class Racetracks : Form
    {
        ...
        private void CreateColumns()
        {
            ...
            dataGridView1.Columns.Add("Owner", "Владелец");
            dataGridView1.Columns.Add("New", String.Empty);
        }

        private void ReadSingleRow(DataGridView dgw, IDataRecord rcord)
        {
            dgw.Rows.Add(rcord.GetInt32(0), rcord.GetString(1), rcord.GetString(2), RowState.ModifiedNew);
        }
        ...
        private void RefreshData(DataGridView dgw)
        {
            ...
            string querystring = $"select * from Racetracks";
        ...
        private void Racetracks_Load(object sender, EventArgs e)
        {
            CreateColumns();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var Owner = textBox2.Text;
            
            if (int.TryParse(textBox9.Text, out id))
            {
                var addQuery = $"insert into Racetracks(ID, Name, Owner) values" +
                               $"('{id}','{Name}', '{Owner}')";
        ...
        private void UpdateDGW()
        {
            ...
            {
                string rowState = Convert.ToString(dataGridView1.Rows[i].Cells[3].Value);
                ...
                if (rowState == "Deleted")
                {
                    ...
                    var deleteQuery = $"delete from Racetracks where id = {id}";
                ...
                if (rowState == "Modified")
                {
                    ...
                    var owner = dataGridView1.Rows[i].Cells[2].Value.ToString();
                    
                    var changeQuery = $"update Racetracks set Name = '{name}', Owner = '{owner}'" +
                        $" where ID = '{id}'";
        ...
        private void Search(DataGridView dgw)
        {
            ...
            string searchString = $"select * from Racetracks where concat (Name, Owner)" +
                $"like '%" + textBox8.Text + "%'";
        ...
        private void Change()
        {
            ...
            if (dataGridView1.Rows[selectedRowI].Cells[0].Value.ToString() != string.Empty)
            {
                if (int.TryParse(textBox9.Text, out id))
                {
                    dataGridView1.Rows[selectedRowI].SetValues(id, Name, Owner);
                                dataGridView1.Rows[selectedRowI].Cells[3].Value = RowState.Modified;
                }
                else
                {
